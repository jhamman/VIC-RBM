...
integer:: nrec_flow,nrec_heat,no_write
integer:: n1,n2,nnd,nobs,ndays,nyear,nd_year,ntmp
integer:: nwpd 
integer,dimension(:),allocatable:: no_dt,nstrt_elm
real::    dt_comp,dt_calc,dt_total,Q1,Q2,q_dot,q_surf,z
real::    T_0,T_dist
real::    x_bndry,x_head
real,dimension(4):: ta,xa
real,dimension(:),allocatable::dt_part,x_part
...
!
! Allocate the arrays and starting element
!
allocate (no_dt(ns_max))
allocate (nstrt_elm(ns_max))
...
!
      call Particle_Track(nr,x_head,x_bndry,dt_part,x_part,no_dt,nstrt_elm)
!
!
        n_write=0
        DONE=.FALSE.
...
!
    if(n_write.ne.ncell) then
              write (20,'(f10.4,4i6,3f8.2,f8.4,f6.1,f9.0,f8.1,f8.4)')  &                                              
                time,nd,nr,ncell,ns,T_0 &
